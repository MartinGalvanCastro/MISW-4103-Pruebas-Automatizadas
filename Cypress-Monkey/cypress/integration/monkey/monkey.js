...
function horizontalScrollBk() {
    ...
    if (curX === 0) {
        ...
        // if(curY === 0) info += "INVALID. Not able to scroll to the bottom anymore. "
        if (viewportWidth > curX) {
            ...
            curX = curX - viewportWidth
            // cy.scrollTo(curX, curY)
            cy.get('.gh-main').scrollTo('bottom', { ensureScrollable: force })
        }
...
function randomEvent() {
    ...
    if (pending_events[typeIndex] > 0) {
        ...
        if(typeIndex >= 0 && typeIndex < functions.length) {
...
describe(`${appName} under monkeys`, function () {
    ...
    it(`visits ${appName} and survives monkeys`, function () {
        ...
        if (pcg === 100) {
            ...
            cy.wait(1000)
            cy.get('form').within(() => {
                cy.get('#identification').type('da.fonseca@uniandes.edu.co')
                cy.get('#password').type('Uniandes.123*')
                cy.get('#ember5').click()
            })
            cy.wait(3000)

            cy.get('.gh-nav-body').contains('Pages').click()

            //Add an event for each type of event in order to enter the else statement of randomEvent method
            for (let i = 0; i < events + 5000000; i++) {
                evtIndex++
                randomEvent()
            }
            //Navigate to the dashboard
            // cy.get('.gh-secondary-action').click()

        }
        else cy.task('logPctNo100')

    })
    it('Send to View Site' , function(){
        if (!seed) seed = getRandomInt(0, Number.MAX_SAFE_INTEGER);
        ...
        if (pcg === 100) {
            ...
            cy.get('.gh-nav-body').contains('View site').click()

            //Add an event for each type of event in order to enter the else statement of randomEvent method
            for (let i = 0; i < events + 50000; i++) {
                evtIndex++
                randomEvent()
            }
    ...
    it(`visits ${appName} Pages`, function () {
        if (!seed) seed = getRandomInt(0, Number.MAX_SAFE_INTEGER);
